...
export default async function handler(req, res) {
  ...
  async function putMethod() {
    try {
      ...
      if(!speakers){
        res.status(404).send("Error: Request failed with a status of 404");
      } else {
  ...
  async function postMethod() {
    try {
      ...
      if(!speakers){
        ...
        const newId = speakers.reduce((accmulator, curr) => {
          const idcurrent = parseInt(curr.id);
          return idcurrent > accmulator ? idcurrent : accmulator;
        }, 0) + 1;
